...
> [!IMPORTANT]
> Please see the new **AWS Resource Cleanup** section below, and make sure you properly delete all of your account resources when submitting Assignment 3.

## ECS Deployment

Your Assignment 3 requires that you complete the [Elastic Container Service (ECS), GitHub Actions, and fragments Walk-Through](fragments-ecs-walkthrough.md). After you have set up your ECS cluster and Load Balancer, you can consider doing the optional [My.Custom.Domain Walkthrough](./mycustomdomain-walkthrough.md) to create a custom domain name, SSL certificate, and secure your service on AWS.

## API Server Checklist

- Clean code
- Docker Compose setup to run `fragments`, DynamoDB Local, and S3 MiniStack containers together in development and GitHub Actions CI workflow for integration testing
...
- Fragments Data stored in Amazon S3, with integration tests using MiniStack container
...
1. A document listing all necessary resources in your system, specifically:
   ...
   - Link to successful GitHub Actions CD workflow run showing integration tests, Docker image publish to Amazon ECR, and deploy to AWS Elastic Container Service
   ...
   - **Screenshots showing that AWS Resources have been deleted** (see **AWS Resource Cleanup** below). At a minimum, this must include proof that you have removed the most expensive resources:
     - **ECS → Clusters** page showing no clusters (or empty list)
     - **EC2 → Load Balancers** page showing no load balancers
     - **VPC → NAT Gateways** page showing no NAT gateways (or all in `Deleted` state)
...
This term we have used CloudLabs for our academic AWS Accounts. In order to stop spending money in your account, you need to **delete your resources manually**. Since CloudLabs does not provide a simple way to delete AWS resources automatically, you will need to do this yourself once your Assignment 3 submission video is prepared.

### End-of-Term AWS Cleanup (Required)

Once you have recorded your assignment walkthrough video, complete these steps **in order**:

1. **Go to ECS → Clusters → {your cluster} → Services**
   - Select your service → **Delete** (type `delete` to confirm)
   - Wait for tasks to drain (~30 seconds)
2. **Go to ECS → Clusters**
   - Select your cluster → **Delete** (type cluster name to confirm)
3. **Go to EC2 → Load Balancers** (left sidebar)
   - Select your ALB → **Actions → Delete**
4. **Go to EC2 → Target Groups** (left sidebar)
   - Select your target group(s) → **Actions → Delete**
5. **Go to VPC → NAT Gateways** (left sidebar)
   - Select your NAT Gateway(s) → **Actions → Delete NAT gateway**
   - Type `delete` to confirm

You should also delete your other resources, which are no longer needed:

1. **Go to Amazon S3**
   - NOTE: you must empty a bucket before you can delete it
   - Select your bucket → **Empty** (type bucket name to confirm)
   - Then select the bucket again → **Delete** (type bucket name to confirm)
2. **Go to DynamoDB → Tables**
   - Select your table → **Delete**
   - Uncheck _"Create a backup before deleting"_ → confirm
3. **Go to Amazon Cognito → User Pools**
   - Select your user pool → **Delete** (type user pool name to confirm)
   - This also removes your app client and hosted UI
4. **Go to Amazon ECR → Repositories**
   - Select your repository → **Delete** (type `delete` to confirm)
5. **Go to CloudWatch → Log Groups** (under Logs in left sidebar)
   - Select any log groups your ECS tasks created (they usually start with `/ecs/`)
   - **Actions → Delete log group(s)**

### Proof of Cleanup

Take **3 screenshots** and include with your assignment document submission:

| Screenshot | What to show                                                                    |
| ---------- | ------------------------------------------------------------------------------- |
| 1          | **ECS → Clusters** page showing no clusters (or empty list)                     |
| 2          | **EC2 → Load Balancers** page showing no load balancers                         |
| 3          | **VPC → NAT Gateways** page showing no NAT gateways (or all in `Deleted` state) |
