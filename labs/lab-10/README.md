...
Next, let's try using the [AWS dynamodb CLI](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/dynamodb/create-table.html).

1. Using the **AWS CLI**, enter the following command:

```sh
aws dynamodb list-tables
```

1. You should see a list of your **Tables**, including the table you created in step 1.
2. Get a description of your `fragments` table with the following command:
...
ddd_v1_w_ysZ_1002077@runweb52179:~$ aws dynamodb describe-table --table-name fragments
{
    "Table": {
        ...
        "TableSizeBytes": 0,
        "ItemCount": 0,
        "TableArn": "arn:aws:dynamodb:us-east-2:423201066495:table/fragments",
        "TableId": "f0a7b457-1432-4394-a736-3e6914e71021"
    }
}
```

1. Notice the definition of your table, with **name** and **type** for your attributes, as well as the definition of your **key**, which uses `ownerId` as the `HASH` key (i.e., **partition key**) and `id` as the `RANGE` key (i.e., **sort key**).
...
1. In our previous lab, we created a new AWS data model in `src/model/data/aws/`, which included an `s3Client.js`. This file configured the [S3 SDK](https://docs.aws.amazon.com/AWSJavaScriptSDK/v3/latest/clients/client-s3/index.html), so that we could issue **commands**. We also wrote this code in such a way that we can easily swap the implementation of S3 that we use at runtime (e.g., use [MiniStack](https://ministack.org/) for testing) via an `endpoint` URL. In this lab we will do a very similar process in order to add [Amazon DynamoDB](https://aws.amazon.com/dynamodb/) support to our `fragments` service. Fist we'll define a **DynamoDB Document Client** named `ddbDocClient.js` and configure it. Then, we'll use it to run **DynamoDB Commands** against our `fragments` DynamoDB table.
...
1. Modify `src/model/data/aws/index.js` to get rid of all uses of `MemoryDB`, since we won't be storying any data in memory, and will use DynamoDB instead. Add a `require()` to use your `ddbDocClient.js` module instead:
...
1. Next, modify the `readFragment` function to use our `ddbDocClient` and the [`GetCommand` from the SDK](https://docs.aws.amazon.com/AWSJavaScriptSDK/v3/latest/classes/_aws_sdk_lib_dynamodb.getcommand-2.html). A DynamoDB `GET` command uses a **Key** to get a DynamoDB **item** and converts it to a JavaScript `Object`.
...
1. Now it's your turn. Follow the same pattern we've used above in order to update the `deleteFragment` function. Currently, it deletes the fragment's metadata from the `MemoryDB` and data from S3. Change it so that the **metadata** gets deleted from S3 **and** DynamoDB.
...
1. In a Unix shell, run the `scripts/local-aws-setup.sh` script to create your `fragments` bucket and table (NOTE: you will need to do this **every** time you (re)start the containers, since they don't store any data to disk):
...
### 7. Integration Testing with MiniStack and DynamoDB in CI
...
1. Modify the final `docker-hub` job so that it also **depends on** the `integration-tests` job:
...
1. Next `add`, `commit`, and `push` everything to GitHub, and make sure your `ci.yml` workflow runs properly. If any of the steps or jobs fail, fix the issues locally and `push` more commits. Repeat until you have a green build, that includes your integration tests.

### 8. Run Locally with AWS S3 and DynamoDB

Now that you know your DynamoDB code works against MiniStack and DynamoDB Local, let's test it against your **real** AWS services. We'll do this by running your `fragments` Docker container locally, but passing in your real AWS credentials as environment variables so the server can connect to your actual S3 bucket and DynamoDB table.

### Passing AWS Credentials to a Local Container

Just as we did in [Lab 9](../lab-09/README.md), we will use our AWS credentials for our CloudLabs environment (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`) to set **environment variables** allowing a local container to use our remote AWS resources:
...
docker run --rm -p 8080:8080 \
  ...
  -e AWS_DYNAMODB_TABLE_NAME=fragments \
...
2. In the **AWS Console** for **DynamoDB**, navigate to your **table** and confirm that the fragment **item** has been created with the correct metadata attributes.

3. In the **AWS Console** for **S3**, navigate to your **bucket** and confirm that the fragment **object** (data) has also been created.
...
- Screenshot of your `tests/integration/lab-10-dynamodb.hurl` test passing, when run against your `fragments` server, `MiniStack`, and `DynamoDB Local` using `docker compose` (i.e., show the terminal(s) running the necessary commands to make this happen).
...
- Screenshot of the **AWS DynamoDB Console** showing the fragment you just created in DynamoDB as an **Item** in your **Table** (i.e., prove that you've been able to create the fragment metadata in your real DynamoDB table)
- Screenshot of the **AWS S3 Console** showing the fragment data you just created in S3 as an object in your bucket
- Confirm that the IDs of the newly created object in S3 and DynamoDB should match.
