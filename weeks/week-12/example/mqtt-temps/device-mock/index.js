...
const topic = `device/${deviceId}/temp`
...
const simulateDeviceReading = () => {
    const data = {
        ...
        temp: faker.datatype.float({ min: 50, max: 100 }),
    ...
    console.log(`deviceId=${deviceId} publish: topic=${topic}, message=${message}`);
    client.publish(topic, message);
};

// Simulate a new temperature reading every 5s
setInterval(simulateDeviceReading, 5 * 1000);
